...
namespace csgo {
	...
	struct entry_point {
		...
		entry_point(T&& single_expression, workgroup wg = {}) : wg(wg) {
			static_assert(std::is_base_of<dsl::layout_variable, meta::unqualified_t<T>>::value, "The return type of a csgo program must be a variable type");
	...
	private:
		...
		entry_point(meta::types<Args...>, std::index_sequence<I...>, T&& multi_expression, workgroup wg) : wg(wg) {
			static_assert(meta::all<std::is_base_of<dsl::layout_variable, meta::unqualified_t<Args>>...>::value, "The return type tuple of a csgo program must all be variables (not expressions)");
			(void)detail::swallow{ (
